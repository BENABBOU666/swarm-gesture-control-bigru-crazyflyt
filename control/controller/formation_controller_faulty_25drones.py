...
- formation_noise (perception noise) with noise level
- state_noise (actuator noise) with noise level
- faulty drones (1% / 2% rules)
...
def run_simulation(
    n_drones=7,
    noise_mode="baseline",   # baseline | formation | state
    fault_mode=False,
    fault_level=0,           # 0 | 1 | 2
    sigma=0.3,
    sim_id=0
    ...
    print(f"[RUN] drones={n_drones}, noise={noise_mode}, fault={fault_mode}")
    ...
    while True:
        ...
        for step in range(num_iterations):
            ...
            if fault_mode:
                for d in faulty:
                    new_pos[d] = positions[d]   # freeze drone
